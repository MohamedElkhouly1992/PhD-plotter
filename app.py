...
def load_file(f):
    ...
    try:
        if ext in ("csv", "txt"):
            ...
            st.sidebar.error(f"Unsupported format: .{ext}"); return

        # Attempt numeric conversion column-by-column without errors="ignore"
        # (deprecated/removed in pandas ≥ 2.2)
        for col in df.columns:
            converted = pd.to_numeric(df[col], errors="coerce")
            # Only apply if at least 80 % of non-null values are numeric
            orig_nn = df[col].notna().sum()
            if orig_nn > 0 and converted.notna().sum() / orig_nn >= 0.8:
                df[col] = converted
